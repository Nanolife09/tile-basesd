...
class Player {
	...
	fall() {
		platformList.forEach(platform => {
			if (checkCollision(player, platform) && this.position.y < platform.position.y) {
	...
	mine() {
		if (control.mine && this.onLand && this.mineCount == this.mineDelay && control.mouseY != undefined) {
			...
			control.mine = false;
			this.mineCount = 0;
			oreList.forEach((ore, index) => {
				if (control.mouseX == ore.position.x && control.mouseY == ore.position.y) {
					ore.currentHardness -= pickaxe[this.pickaxe].dmg;
					if (ore.currentHardness > 0) {
						textList.push(new Text({
							text: `${ore.currentHardness} / ${ore.maxHardness}`, 
							position: {
								x: ore.position.x + ore.width / 2,
								y: ore.position.y - 10
							}
						}));
					}
					...
					if (ore.currentHardness <= 0) {
						emptyTileList.push([ore.position.x, ore.position.y]);
						oreList.splice(index, 1);
						oreBreakSound.play();
						var chunks = Math.floor(Math.random() * (oreType[ore.block].maxDrop - oreType[ore.block].minDrop) + oreType[ore.block].minDrop);
						for (let chunk = 0; chunk < chunks; chunk++) {
							dropItemList.push(new OreChunk({
								position: {
									x: ore.position.x + ore.width / 2, 
									y: ore.position.y - ore.height / 2
								},
								velocity: {
...
class OreChunk {
	...
	fall() {
		platformList.forEach(platform => {
			if (checkCollision(platform, this) && this.position.y < platform.position.y) {
				this.onLand = true;
				this.position.y = platform.position.y - this.height;
			}
		});
...
class Ore {
	...
	draw() {
		ctx.drawImage(oreImage, oreType[this.block].tier * 64, 0, 64, 64, this.position.x, this.position.y, this.width, this.height);
		ctx.fillStyle = "rgba(255, 0, 0, 0.2)";
		ctx.fillRect(this.position.x, this.position.y + tileSize * (this.currentHardness / this.maxHardness), tileSize, tileSize * (1 - this.currentHardness / this.maxHardness));
	}
}
...
class Platform {
	...
	draw() {
		ctx.drawImage(platformImage, 0, 0, 64, 32, this.position.x, this.position.y, this.width, this.height);
	}
}
...
class GUI {
	constructor() {
		this.position = {
			x: 0,
			y: canvas.height - guiGridSize + tileSize / 2
		};
		this.width = canvas.width;
		this.height = canvas.height - guiGridSize - tileSize / 2;
	}
}
...
function createEmptyCell() {
	for (let row = 0; row < canvas.height - guiGridSize; row += tileSize) {
		for (let column = 0; column < canvas.width; column += tileSize) {
...
function createOre() {
	for (let i = 0; i < canvas.width * (canvas.height - guiGridSize)/ Math.pow(tileSize, 2); i++) {
...
function mineRegen() {
	...
	if (currentMineRegenTime == mineRegenTime && emptyTileList.length > 0) {
		var tile = emptyTileList[Math.floor(Math.random() * (emptyTileList.length - 1))];
		var abundance = Math.floor(Math.random() * 100);
		var currentBlock;
		for (let ore in oreType) {
			if (oreType[ore].minAbundance <= abundance && oreType[ore].maxAbundance >= abundance) {
...
canvas.addEventListener("mousemove", event => {
	control.mouseX = (event.x - canvasBounding.left) - ((event.x - canvasBounding.left) % tileSize);
	control.mouseY = (event.y - canvasBounding.top) - ((event.y - canvasBounding.top) % tileSize);
	if (control.mouseY >= canvas.height - guiGridSize) {
...
window.addEventListener("resize", event => {
	...
	canvas.height = Math.round(mapHeight * tileSize);
	canvas.width = Math.round(mapWidth * tileSize);
